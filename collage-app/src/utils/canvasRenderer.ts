...
function drawCheckerboard(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
  ...
  for (let cx = x; cx < x + w; cx += size) {
    for (let cy = y; cy < y + h; cy += size) {
      ...
      ctx.fillStyle = isEven ? '#e8e8e8' : '#f4f4f4'
...
function drawPlaceholder(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
  ...
  const iconSize = Math.max(16, Math.min(36, minDim * 0.16))
  const fontSize = Math.max(11, Math.min(13, minDim * 0.05))
  const gap = minDim * 0.04
  const groupH = hasText ? iconSize + gap + fontSize : iconSize
  const iconCY = cy - groupH / 2 + iconSize / 2

  const lw = Math.max(1.2, minDim * 0.018)
